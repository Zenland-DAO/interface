...
export function AgentsClient({ isSelectMode = false }: AgentsClientProps) {
  ...
  const SORT_OPTIONS = [
    { label: t("sortOptions.reputation"), value: "reputation" },
    { label: t("sortOptions.stake"), value: "stake" },
    { label: t("sortOptions.fees"), value: "fees" },
    { label: t("sortOptions.cases"), value: "cases" },
  ];
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <PageHeader
          title={isSelectMode ? t("selectTitle") : t("title")}
          description={isSelectMode ? t("selectDescription") : t("description")}
        ...
        <Card variant="outlined" className="border-error-200 dark:border-error-900/30">
          <CardBody className="p-12 text-center flex flex-col items-center gap-4">
            ...
            <div className="space-y-1">
              ...
              <Text variant="muted">{t("error.description")}</Text>
            </div>
            <Button variant="outline" onClick={() => window.location.reload()}>{t("error.retry")}</Button>
          </CardBody>
        </Card>
      </div>
    );
          ...
          <div className="flex items-center gap-3 p-4 rounded-xl bg-primary-50 dark:bg-primary-900/10 border border-primary-200 dark:border-primary-800">
            ...
            <div className="flex-1">
              <Text className="font-semibold text-primary-800 dark:text-primary-200">
                {t("selectModeBanner.title")}
              </Text>
              <Text variant="muted" className="text-sm">
                {t("selectModeBanner.description")}
              </Text>
            </div>
          </div>
        )}
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <div className="flex items-center gap-1.5">
                            <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded-md border ${
                              ...
                              {agent.isActive ? t("card.active") : t("card.inactive")}
                            </span>
                    ...
                    <div className="mt-4 p-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/50 border border-[var(--border-secondary)]">
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5 text-[var(--text-tertiary)]">
                            <Wallet size={12} />
                            <span className="text-[10px] font-bold uppercase tracking-wider">{t("card.stablecoin")}</span>
                          </div>
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5 text-[var(--text-tertiary)]">
                            <Coins size={12} />
                            <span className="text-[10px] font-bold uppercase tracking-wider">{t("card.daoToken")}</span>
                          </div>
                          <Text className="font-bold text-sm">
                      ...
                      <div className="mt-3 pt-3 border-t border-[var(--border-secondary)]">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1.5 text-[var(--text-tertiary)]">
                            <TrendingUp size={12} />
                            <span className="text-[10px] font-bold uppercase tracking-wider">{t("card.maxCoverage")}</span>
                          </div>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-3">
                      <div className="p-2.5 rounded-lg border border-[var(--border-secondary)] text-center">
                        <div className="flex items-center justify-center gap-1 text-[var(--text-tertiary)] mb-1">
                          ...
                          <span className="text-[9px] font-bold uppercase tracking-wider">{t("card.disputeFee")}</span>
                        </div>
                      ...
                      <div className="p-2.5 rounded-lg border border-[var(--border-secondary)] text-center">
                        <div className="flex items-center justify-center gap-1 text-[var(--text-tertiary)] mb-1">
                          <FileText size={10} />
                          <span className="text-[9px] font-bold uppercase tracking-wider">{t("card.assignFee")}</span>
                        </div>
                          ...
                          {agent.registrationTime && (
                            <div className="flex items-center gap-1.5 text-[var(--text-tertiary)]">
                              ...
                              <span className="text-[10px] font-bold uppercase tracking-wider">
                                {t("card.joined", { date: new Date(Number(agent.registrationTime) * 1000).toLocaleDateString("en-US", { month: "short", year: "numeric" }) })}
                              </span>
                            </div>
                          )}
