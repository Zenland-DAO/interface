...
export function SplitNegotiation({ onOpenModal }: SplitNegotiationProps) {
  const t = useTranslations("escrows.detail.split");
  const tActions = useTranslations("escrows");
          ...
          {hasActiveProposal && splitProposal && (
            <div className="p-3 bg-primary-50/50 dark:bg-primary-900/10 rounded-lg space-y-3">
              <div className="flex items-center justify-between">
                <Text className="text-sm font-semibold">{t("currentProposal")}</Text>
                <Text variant="muted" className="text-xs">
                  {t("by", { address: `${splitProposal.proposer?.slice(0, 6)}...` })}
                </Text>
              </div>
              ...
              <div className="flex justify-between text-xs pt-2 border-t border-[var(--border-secondary)]">
                <div className="flex items-center gap-1">
                  ...
                  <span>{splitProposal.buyerApproved ? t("buyerApproved") : t("buyerPending")}</span>
                </div>
                <div className="flex items-center gap-1">
                  ...
                  <span>{splitProposal.sellerApproved ? t("sellerApproved") : t("sellerPending")}</span>
                </div>
              </div>
                  ...
                  >
                    {isPending && pendingAction === "approveSplit" ? (
                      <>
                        ...
                        {tActions("actionProgress.approving")}
                      </>
                    ) : isConnected ? (
                      t("approveSplit")
                    ) : (
                      <>
                        ...
                        {tActions("actions.connectWallet")}
                      </>
                    )}
                  </Button>
                )}
              ...
              >
                {isPending && pendingAction === "proposeSplit" ? (
                  <>
                    ...
                    {tActions("actionProgress.proposing")}
                  </>
                ) : isConnected ? (
                  <>
